...
export type PebbleVariant = 0 | 1 | 2 | 3 | 4 | 5;
...
// Pebble silhouette paths (100×100 viewBox). Roles by index:
//   0: rounded triangle, apex top    — feathers
//   1: rounded oval / circle         — yarn
//   2: squircle (rounded square)     — landscapes
//   3: rounded triangle, lean        — feathers (alt)
//   4: rounded rectangle, squat      — landscapes (alt)
//   5: irregular pebble              — yarn (alt)
...
export function Pebble({ className, variant = 0, children }: PebbleProps) {
  // useId guarantees a unique gradient id per Pebble instance. A static id
  // (`pebbleShine-${variant}`) would make every same-variant pebble share one
  // global id — when one unmounted, the others' `url(#…)` refs would break
  // and paths would render transparent.
  const reactId = useId();
  const gradientId = `pebbleShine-${reactId.replace(/[^a-zA-Z0-9_-]/g, "")}`;
  return (
    <div className={cn("relative", className)}>
      <svg
        ...
        aria-hidden
        style={{ filter: PEBBLE_SHADOW }}
      >
        <defs>
          <radialGradient id={gradientId} cx="0.3" cy="0.25" r="0.75">
